...
<section class="section">
  <div class="section-header">
    ...
    <div class="section-header-breadcrumb">
      ...
      <div class="breadcrumb-item"><a href="<?=base_url('aplikasi_modul/akses')?>">Components</a></div>
      <div class="breadcrumb-item">Akses</div>
    </div>
  </div>

  <div class="section-body">
    <h2 class="section-title">Akses</h2>
    <p class="section-lead">atur pegawai untuk aplikasi <?=$title?></p>
...
<script type="text/javascript">
    ...
    $(document).ready( function () {
      table = $('#table-module').DataTable({ 
        ...
        "ajax": {
          "url": "<?php echo site_url('aplikasi_modul/akses/get_data')?>",
          "type": "POST"
        },
        "columnDefs": [
        { 
    ...
    $(document).on('click',"#add-button",function(){
        $.ajax({
          url: '<?php echo site_url('aplikasi_modul/akses/modal'); ?>',
          type: 'post',
    ...
    $(document).on("submit", "#form-save", function(e) {
        e.preventDefault();
        $.ajax({
            url: '<?=site_url('aplikasi_modul/akses/save');?>',
            data: $(this).serialize(),
            dataType: 'json',
            type: 'post',
            beforeSend: function(){
    ...
    function change_state(id,status){
     ...
     function delete_level(id){
       ...
       Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
                url: '<?=site_url('aplikasi_modul/akses/delete');?>',
                data: {id:id},
                dataType: 'json',
                type: 'post',
    ...
    $(document).on('change','#id_instansi',function(){
      ...
      if(id_instansi != ''){
        $.ajax({
          url: '<?=site_url('aplikasi_modul/akses/get_pegawai');?>/'+id_instansi,
          dataType: 'json',
            beforeSend: function(){
                $('#progress').show();
            },
